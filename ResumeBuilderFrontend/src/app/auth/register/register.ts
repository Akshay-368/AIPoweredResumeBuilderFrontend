...
export class Register implements AfterViewInit {
  ...
  isGoogleSubmitting: boolean = false;
  errorMessage: string = '';
  ...
  private initializeGoogleButton(): void {
    const hostElement = this.googleRegisterButtonHost?.nativeElement;
    ...
    this.authService.getGoogleClientConfig().subscribe({
      next: (config) => {
        google.accounts.id.initialize({
          ...
          callback: (response: { credential?: string }) => {
            ...
            if (!idToken) {
              this.errorMessage = 'Google sign-up failed: missing credential token.';
              this.cdr.detectChanges();
              return;
            }
      ...
      error: () => {
        this.errorMessage = 'Google sign-up is not configured on server.';
        this.cdr.detectChanges();
      }
    });
  }
